...
public class TurretSubsystem extends SubsystemBase {
  ...
  SparkMax turretMotor, shootingMotorLeft, shootingMotorRight;
  RelativeEncoder shooterEncoder, turretEncoder;
  ...
  private void setupMotorsAndEncoders() {
    ...
    turretConfig.smartCurrentLimit(Constants.neoSafeAmps);
    // Set up Motors
    this.turretMotor = new SparkMax(Constants.turretSparkMax, MotorType.kBrushless);
    this.turretMotor.configure(turretConfig, ResetMode.kNoResetSafeParameters, PersistMode.kNoPersistParameters);


    
    SparkMaxConfig shooterConfig = new SparkMaxConfig();
    shooterConfig.idleMode(IdleMode.kCoast);
    shooterConfig.smartCurrentLimit(Constants.neoSafeAmps);
    this.shootingMotorLeft = new SparkMax(Constants.shooterSparkMaxLeft, MotorType.kBrushless);
    this.shootingMotorLeft.configure(shooterConfig, ResetMode.kNoResetSafeParameters, PersistMode.kNoPersistParameters);

    this.shootingMotorRight = new SparkMax(Constants.shooterSparkMaxRight, MotorType.kBrushless);
    this.shootingMotorRight.configure(shooterConfig, ResetMode.kNoResetSafeParameters, PersistMode.kNoPersistParameters);
